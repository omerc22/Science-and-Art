...
if mode == "Chat Mode":
    ...
    if 'generated_image' in st.session_state:
        ...
        filter_type = st.selectbox(
            ...
            ["Original", "Grayscale", "Blur", "Contour", "Sepia", "Sharpen", "Invert"]
        )
        ...
        if filter_type == "Grayscale":
            ...
            processed_image = processed_image.filter(ImageFilter.GaussianBlur(radius=3))
        elif filter_type == "Contour":
            processed_image = processed_image.filter(ImageFilter.CONTOUR)
        elif filter_type == "Sepia":
            gray = ImageOps.grayscale(processed_image)
            processed_image = ImageOps.colorize(gray, "#704214", "#C0C0C0")
        ...
        st.image(processed_image, caption=f"Prompt: {st.session_state.get('last_prompt', '')} | Filter: {filter_type}", use_container_width=True)
